...
Popcorn — a movie/TV rating app. Users rate titles on a 0–6 "popcorn bag" scale in half-bag increments, keep a personal watched timeline, and follow other users for a basic activity feed. All 5 MVP phases are implemented: auth, TMDB search/detail, ratings CRUD + personal timeline, the social layer (follow/unfollow, activity feed, comments, user discovery), and polish/deploy (accessibility, responsive, Docker, CI/CD). GCP/Firebase infrastructure is provisioned and GitHub Actions secrets are set — see "Deploy pipeline" below for the details and current status.
...
Real secrets live in `server/.env` and `client/.env` (both gitignored, never commit them). `.env.example` at the root documents every variable and where to obtain it; `server/.env.example` just points back to the root file. Required for the server to boot: `DATABASE_URL`/`DIRECT_URL` (Neon), `FIREBASE_PROJECT_ID`/`FIREBASE_CLIENT_EMAIL`/`FIREBASE_PRIVATE_KEY_BASE64` (Firebase Admin service account), `TMDB_API_KEY` (themoviedb.org). The client needs the matching `VITE_FIREBASE_*` values from the same Firebase project's Web App config, plus `VITE_API_BASE_URL`.

**`FIREBASE_PRIVATE_KEY_BASE64` is base64-encoded, not a raw `\n`-escaped PEM** — `firebaseAdmin.ts` does `Buffer.from(env.FIREBASE_PRIVATE_KEY_BASE64, "base64").toString("utf8")`. This isn't a style preference: a raw multi-line-when-decoded PEM value broke a real production deploy. `deploy.yml`'s Cloud Run step interpolates secrets into a `env_vars: |` YAML block that the `deploy-cloudrun` action parses one `KEY=VALUE` per line; a `\n`-escaped single-line secret still went through *something* in that pipeline (GitHub's expression substitution, the action's own parser, or gcloud's `--update-env-vars` flag parsing — never fully isolated which) that stripped the backslashes, leaving literal `n` characters instead of either `\n` or a real newline, producing "Invalid PEM formatted message" and a Cloud Run container that failed to start within its health-check timeout. Base64 has no special characters for any of those layers to mangle, which is why it's the fix rather than a formatting nitpick. If this env var is ever renamed or this decoding logic changes, re-verify against an actual Cloud Run deploy, not just local `npm run dev` — the corruption only manifested in the YAML-interpolation path, not locally where `dotenv` parses `server/.env` directly.
...
- `$color-primary` in `_variables.scss` is `#c2410c`, not a brighter orange, specifically because it's used as link/text color throughout (`a { color: $color-primary }` in `global.scss`) — a brighter shade measured under WCAG AA's 4.5:1 contrast threshold against `$color-bg`; this one clears it (re-verified at ~4.80:1 against the current cream `$color-bg`, see "Visual design system" below). Don't brighten it without rechecking contrast.
...
### Visual design system: "vintage cinema marquee"

The UI follows a retro cinema theme — warm parchment/cream backgrounds, a burnt-orange/mustard-gold/maroon accent palette, and a condensed marquee-style display font for headings — layered on top of the pre-existing `$color-primary` burnt orange rather than replacing it. All tokens live in `_variables.scss`; component styles should pull from these, not hardcode new hex values or radii.

- **Colors**: `$color-bg` (`#fdf6e3` cream) / `$color-surface` (`#fffaf0`, slightly lighter — used for card/input faces so they read as distinct from the page background without relying on shadow alone) / `$color-text` (`#2b1810` espresso) / `$color-muted` (`#6b5842`) / `$color-border` (`#d8c3a0`, structural/decorative only — never used as a text color) / `$color-primary` (`#c2410c`, unchanged) / `$color-gold` (`#d4a017`) / `$color-maroon` (`#7a1f1f`). **`$color-gold` fails WCAG AA as small text on `$color-bg` (2.20:1) — it's decoration-only** (film-strip stripes, badge fills with dark text on top), never a text color on the page background. All other pairings were verified ≥4.5:1 by computing actual relative-luminance contrast, not eyeballed — re-verify computationally, not visually, if any of these values ever change.
- **Radii/shadows**: `$radius-sm`/`$radius-md` replace old hardcoded `4px`/`8px`/`6px` values. `$shadow-ticket` (`4px 4px 0 $color-maroon`) is a hard, blur-less offset shadow — the "printed ticket stub" look — applied via the `ticket-card` mixin (`_mixins.scss`), not written ad hoc per component.
- **Typography**: `$font-display` (Bebas Neue, loaded via a Google Fonts `<link>` in `client/index.html` with `display=swap` + `preconnect` hints — no npm dependency) is used **only** for site chrome (global `h1`/`h2` in `global.scss`, `Header`'s `.brand`), always uppercase. It is deliberately **not** applied to card titles (movie names, usernames in `TitleCard`/`RatingCard`/`ActivityItem`/`UserCard`) — a condensed all-caps face hurts legibility on long, variable-length real-world titles; those stay on `$font-body` (`system-ui`, unchanged) at their existing weight.
- **`ticket-card` mixin** (`_mixins.scss`): the boxed card face (`$color-surface` background + border + `$radius-md` + `$shadow-ticket`) shared by `RatingCard`, `ActivityItem`, `UserCard`, `TitleCard`, and `RatingForm`. Add `@include ticket-card;` to a container rather than duplicating the four declarations — this is why those five components were converted from flat list-rows (a single `border-bottom`) to stacked boxed cards with `margin-bottom` for spacing; that layout shift (list rows → ticket-stub cards) was deliberate, not a partial migration.
- **Film-strip motif**: a repeating sprocket-hole gradient (`repeating-linear-gradient` alternating `$color-maroon`/`$color-gold`) used as a thin decorative band in two places — full-width under `Header` (via `.header::after`, added as a normal-flow block sized by negative margins matching the header's own padding, not absolutely positioned, so it can't overlap page content below it) and as a short flourish above every `h2` (`global.scss`, `h2::before`) marking section headings like "Cast" on the title detail page. There's no shared `.film-strip` class — it's two small, non-identical declarations, not worth abstracting.
- **Favicon**: `client/public/favicon.svg` is a hand-rolled inline SVG (the codebase already hand-rolls the `PopcornRating` bag icons the same way) using literal hex values matching the palette above — SVGs can't reference SCSS variables, so if the palette changes, this file needs its hex values updated by hand to match.
...
`.github/workflows/ci.yml` (lint/typecheck/test/build, called both directly on PRs and as a reusable `workflow_call` job from `deploy.yml`) and `.github/workflows/deploy.yml` (on push to `main`: gate → `prisma migrate deploy` against production Neon → build+push `server/Dockerfile` to Artifact Registry → `gcloud run deploy` → build the client with the live Cloud Run URL as `VITE_API_BASE_URL` → `firebase deploy --only hosting`) both exist in the repo.

Infra is provisioned on GCP project `popcorn-70873` (same project as the Firebase project — Firebase projects ARE GCP projects): billing linked, `run`/`artifactregistry`/`iam`/`firebasehosting` APIs enabled, an Artifact Registry Docker repo named `popcorn` in `us-central1`, and a single service account `popcorn-deployer@popcorn-70873.iam.gserviceaccount.com` (roles: Artifact Registry Writer, Cloud Run Admin, Service Account User, Firebase Hosting Admin) whose JSON key is stored as the `GCP_SA_KEY` GitHub secret and reused for **both** the Cloud Run deploy step and the `FirebaseExtended/action-hosting-deploy` step — deliberately one service account, not two, to keep provisioning simpler (see the comment block at the top of `deploy.yml` for the full secret list). Production currently reuses the same Neon database as local dev (not a separate prod DB) — a deliberate simplicity tradeoff for a single-developer project, revisit with a Neon branch if that ever becomes a problem. `server/Dockerfile` has been verified locally (not just "should work"): built the image, ran the container against the real Neon DB and TMDB key, confirmed reads/writes succeed — that verification is what caught the build-script-ordering bug noted above, and separately caught that Prisma's query-engine binary target detection needs `openssl` installed in the *build* stage too, not just the runtime stage (alpine/musl), or it silently defaults to a possibly-mismatched target.
